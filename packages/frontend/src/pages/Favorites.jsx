...
const getUserFavs = async (username) => {
  try {
    ...
    console.log("user favorite ids:", user[0].favorites)
    return user[0].favorites
  }
  catch (error) {
    console.log("Unable to successfully retreive user favs:", error)
  }
}
...
function Favorites() {
  ...
  useEffect(() => {
    getUserFavs(username).then(res => {
      ...
      Promise.all(res.map(id => getListings(id).then(item => item.data[0])))
